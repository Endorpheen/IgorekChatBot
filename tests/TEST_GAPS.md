...
### ✅ Добавленные unit тесты (+37 тестов):
- **Image generation router** (`tests/unit/test_image_generation_router.py`): 19 тестов для Pydantic моделей, валидации запросов, параметров, URL редиректов, обработки ошибок, Rate limiting, dependency injection
...
- **Всего unit тестов**: 204 → **223** (+19 тестов, 100% стабильность)
...
- Продолжить с целью ≥55%: выбрать следующие модули с низким покрытием
- `app/features/webui.py` (24%, 31 непокрытых строк) - следующие на очереди
- `app/features/image_analysis/router.py` (33%, 70 непокрытых строк)
- `app/features/chat/router.py` (43%, 79 непокрытых строк)
