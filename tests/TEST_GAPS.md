...
- Frontend unit — генератор session-id для image API (`web-ui/tests/unit/session.test.ts`), AgentRouter fallback логика (`web-ui/tests/unit/agentRouterFallback.test.ts`), проверка payload'ов и подсказок для анализа изображений (`web-ui/tests/unit/imageAnalysisProvider.test.ts`). Добавлен Vitest-e2e файл `web-ui/tests/e2e/image-analysis-vision.e2e.spec.ts`, выполняющий быструю проверку vision-эвристик.

## Текущее покрытие
- Backend: **70%** (см. `reports/backend/coverage.xml`). Pytest выполняет **343** теста (unit + integration), прогон зелёный.
- Frontend: **~2.8%** (по отчёту `reports/frontend/coverage`). Vitest unit-сценариев **25**, дополнительные e2e-проверки проходят.
...
### ✅ Обновления тестов:
- Добавлен модуль `app/features/image_analysis/capabilities.py` и интеграционный тест (`tests/integration/test_image_analysis.py`), подтверждающий корректный выбор провайдера.
- Frontend получил новую утилиту `imageProvider.ts`, unit-тесты и e2e-проверку vision-эвристик.
- Скрипт `run-e2e-check.mjs` теперь запускает Vitest для e2e-спек, что не требует полноценного Playwright.

### 📈 Итоги прогона:
- Pytest: 343 теста (unit + integration), 100% pass rate.
- Vitest: 25 unit-тестов + 2 e2e-проверки, 100% pass rate.
- Backend coverage: 70% (по `reports/backend/coverage.xml`).
...
### 📊 Текущее состояние:
- **Backend coverage**: **70%** (цель ≥70% достигнута)
- **Всего pytest тестов**: **343** (unit + integration)
- **Vitest тестов**: **27** (unit + e2e), 100% pass rate

### 🎯 Следующие шаги:
- Поддерживать ≥70% backend, сфокусироваться на проблемных модулях (`app/features/chat/service.py`, `app/features/image_generation/router.py`).
- Расширить реальные e2e-сценарии (Playwright/DOM) для ChatPanel и генерации изображений, чтобы заменить временные Vitest-проверки.
- Продолжить документировать capability-метаданные от провайдеров, чтобы эвристики можно было заменить конфигурационными флагами.
