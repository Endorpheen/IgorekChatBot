...
def generate_signed_token(file_path="test.txt"):
    """Генерируем простой токен для signed эндпоинта"""
    timestamp = str(int(time.time()))
    signature = hashlib.md5(f"{file_path}:{timestamp}:test_secret".encode()).hexdigest()
    return f"{timestamp}:{signature}"

def test_real_attachment_endpoint():
    """Тестирование реального /signed/chat/attachments эндпоинта"""
    print("🔥 ТЕСТИРОВАНИЕ РЕАЛЬНОГО /signed/chat/attachments ЭНДПОИНТА")
    print("=" * 60)

    # Тестовые данные
    test_token = generate_signed_token("test.txt")
    test_params = {
        "token": test_token,
        "file_path": "test.txt"
    }
    ...
    for i in range(10):
        try:
            start = time.time()
            response = requests.post(f"{BASE_URL}/signed/chat/attachments",
                                   params=test_params,
            ...
            if i <= 2:  # Только первые 3 ответа
                try:
                    data = response.json()
                    print(f"    📝 Ответ: {data.get('detail', data)[:100]}")
                except:
    ...
    for i in range(50):
        try:
            # Генерируем новый токен для каждого запроса (более реалистично)
            test_token = generate_signed_token(f"test_file_{i}.txt")
            test_params = {"token": test_token, "file_path": f"test_file_{i}.txt"}

            start = time.time()
            response = requests.post(f"{BASE_URL}/signed/chat/attachments",
                                   params=test_params,
                                   json={"file_name": f"test_file_{i}.txt"},
                                   timeout=15)
            end = time.time()
    ...
    if response_times:
        ...
        print(f"\n📊 РЕАЛЬНАЯ СТАТИСТИКА /signed/chat/attachments:")
...
def main():
    print("🚀 НАГРУЗОЧНОЕ ТЕСТИРОВАНИЕ С РЕАЛЬНОЙ БИЗНЕС-ЛОГИКОЙ")
    print(f"📅 Время: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print("=" * 70)

    # Тестируем основной эндпоинт
    result1 = test_real_attachment_endpoint()
